...
2. **Interkvartilis terjedelem (IQRIQR):**
    - $IQR = Q_3 - Q_1$
...
Pozitív adatsokaság ${a_1, a_2, ..., a_n\}$ ahol n≥2n \geq 2) nevezetes közepei:

1. **Számtani közép (A(ai)A(a_i)):**
    
    A(ai)=1n∑i=1naiA(a_i) = \frac{1}{n} \sum_{i=1}^n a_i
2. **Mértani közép (G(ai)G(a_i)):**
    
    G(ai)=a1⋅a2⋅...⋅annG(a_i) = \sqrt[n]{a_1 \cdot a_2 \cdot ... \cdot a_n}
3. **Harmonikus közép (H(ai)H(a_i)):**
    
    H(ai)=n∑i=1n1aiH(a_i) = \frac{n}{\sum_{i=1}^n \frac{1}{a_i}}
4. **Négyzetes (kvadratikus) közép (Q(ai)Q(a_i)):**
    
    Q(ai)=1n∑i=1nai2Q(a_i) = \sqrt{\frac{1}{n} \sum_{i=1}^n a_i^2}

---

#### **2. Összefüggések**

- **Nevezetes középek sorrendje:** min⁡(ai)≤H(ai)≤G(ai)≤A(ai)≤Q(ai)≤max⁡(ai)\min(a_i) \leq H(a_i) \leq G(a_i) \leq A(a_i) \leq Q(a_i) \leq \max(a_i)
- **Egyenlőség feltétele:**  
    Az egyenlőség akkor és csak akkor teljesül, ha a1=a2=...=ana_1 = a_2 = ... = a_n.

---

#### **3. Tétel (Két pozitív szám harmonikus és számtani közepe közötti kapcsolat)**

**Állítás:**  
Ha a,b∈R+a, b \in \mathbb{R}^+, akkor:

H(a,b)≤A(a,b)H(a, b) \leq A(a, b)

**Bizonyítás:**

1. Harmonikus közép:
    
    H(a,b)=21a+1b=2aba+bH(a, b) = \frac{2}{\frac{1}{a} + \frac{1}{b}} = \frac{2ab}{a + b}
2. Számtani közép:
    
    A(a,b)=a+b2A(a, b) = \frac{a + b}{2}
3. Összehasonlítás:
    
    2aba+b≤a+b2\frac{2ab}{a + b} \leq \frac{a + b}{2}
    
    Szorzunk (a+b)(a + b)-vel (pozitív érték):
    
    4ab≤(a+b)24ab \leq (a + b)^2
    
    Felbontva:
    
    4ab≤a2+2ab+b24ab \leq a^2 + 2ab + b^2 0≤(a−b)20 \leq (a - b)^2
    
    Ez mindig igaz. Az egyenlőség a=ba = b esetén áll fenn.
...
A nevezetes közepek gyakorlati jelentősége abban áll, hogy különböző eloszlások és folyamatok jellemzésére, valamint elemzési célokra széles körben alkalmazhatók.