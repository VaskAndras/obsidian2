...
V=13πm(R2+Rr+r2)V = \frac{1}{3} \pi m \left( R^2 + Rr + r^2 \right)

ahol RR az alapkör sugara, rr a fedőkör sugara és mm a csonka körkúp magassága.

---

### **Bizonyítás**:

A csonka körkúp térfogatának meghatározása során egy lineáris függvény segítségével fogunk integrálni. A csonka kúp egy olyan test, amelyet az xx-tengely körül történő forgatás ad. A test térfogatát a következő alapelv segítségével számíthatjuk ki, ha a függvény körüli forgatásból származó térfogatot akarjuk meghatározni:

### **1. Alapvető térfogat képlet**:

Ha egy f(x)f(x) függvényt az [a,b][a, b] intervallumon integrálunk, és f(x)≥0f(x) \geq 0 a teljes intervallumra, akkor a forgatás által keletkezett térfogatot az alábbi képlettel számíthatjuk ki:

V=π∫abf(x)2 dxV = \pi \int_a^b f(x)^2 \, dx

Ez a képlet a forgástestek térfogatának meghatározására szolgál, amikor a test a xx-tengely körül forog.

### **2. A csonka kúp meghatározása**:

A csonka kúp egy kúp, aminek az alja és a teteje is kör. A csonka kúpokat egy lineáris függvénnyel modellezhetjük, amely az alapkör és a fedőkör sugara között változik. A csonka kúp magassága mm, az alapkör sugara RR, a fedőkör sugara pedig rr.
...
Ahol aa és bb a függvény paraméterei, amelyeket az alábbi két pont segítségével határozunk meg:

- Az x=0x = 0 helyen a sugár rr, azaz f(0)=rf(0) = r,
- Az x=mx = m helyen a sugár RR, azaz f(m)=Rf(m) = R.

### **3. A lineáris függvény paramétereinek meghatározása**:

A két pont alapján meghatározhatjuk a lineáris függvény paramétereit:

- Az f(0)=rf(0) = r alapján b=rb = r,
- Az f(m)=Rf(m) = R alapján pedig R=am+rR = am + r, amiből kifejezhetjük aa-t: a=R−rma = \frac{R - r}{m}

Ezért a lineáris függvényünk:

f(x)=R−rmx+rf(x) = \frac{R - r}{m}x + r

### **4. A térfogat meghatározása**:

Miután meghatároztuk a lineáris függvényt, a csonka kúp térfogatának meghatározása következik, amely a függvény körüli forgatásból származik. A térfogatot az alábbi integrál segítségével számíthatjuk ki:

V=π∫0mf(x)2 dxV = \pi \int_0^m f(x)^2 \, dx

A függvényt behelyettesítve:

V=π∫0m(R−rmx+r)2 dxV = \pi \int_0^m \left( \frac{R - r}{m}x + r \right)^2 \, dx

Most a kifejezést kibővítjük, hogy könnyen integrálható legyen:

V=π∫0m[(R−rmx)2+2⋅(R−rmx)r+r2]dxV = \pi \int_0^m \left[ \left( \frac{R - r}{m}x \right)^2 + 2 \cdot \left( \frac{R - r}{m}x \right)r + r^2 \right] dx

Ez felírható az alábbi három integrál formájában:

V=π(∫0m(R−rmx)2dx+2r∫0m(R−rmx)dx+∫0mr2dx)V = \pi \left( \int_0^m \left( \frac{R - r}{m}x \right)^2 dx + 2r \int_0^m \left( \frac{R - r}{m}x \right) dx + \int_0^m r^2 dx \right)
...
∫0m(R−rmx)2dx=(R−r)2m2∫0mx2dx=(R−r)2m2⋅m33=(R−r)2m3\int_0^m \left( \frac{R - r}{m}x \right)^2 dx = \frac{(R - r)^2}{m^2} \int_0^m x^2 dx = \frac{(R - r)^2}{m^2} \cdot \frac{m^3}{3} = \frac{(R - r)^2 m}{3}

- A második integrál:

∫0m(R−rmx)dx=R−rm∫0mxdx=R−rm⋅m22=(R−r)m2\int_0^m \left( \frac{R - r}{m}x \right) dx = \frac{R - r}{m} \int_0^m x dx = \frac{R - r}{m} \cdot \frac{m^2}{2} = \frac{(R - r) m}{2}

- A harmadik integrál:

∫0mr2dx=r2m\int_0^m r^2 dx = r^2 m

### **6. Az eredmény egyesítése**:

Most, hogy kiszámoltuk az integrálokat, összegezzük őket:

V=π[(R−r)2m3+2r⋅(R−r)m2+r2m]V = \pi \left[ \frac{(R - r)^2 m}{3} + 2r \cdot \frac{(R - r) m}{2} + r^2 m \right]

Egyszerűsítve:

V=πm[(R−r)23+r(R−r)+r2]V = \pi m \left[ \frac{(R - r)^2}{3} + r(R - r) + r^2 \right]

Most a kifejezést kibővítve és egyszerűsítve:

V=πm(R2−2Rr+r23+Rr−r2+r2)V = \pi m \left( \frac{R^2 - 2Rr + r^2}{3} + Rr - r^2 + r^2 \right) V=πm(R2+Rr+r23)V = \pi m \left( \frac{R^2 + Rr + r^2}{3} \right)
